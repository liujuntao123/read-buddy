...
beforeEach(() => {
  ...
  window.localStorage.setItem('read-buddy:page-mode', 'double');
...
describe('ReaderDock', () => {
  ...
  it('toggles page mode through the store and the pane applies it to the engine', async () => {
    ...
    expect(window.localStorage.getItem('read-buddy:page-mode')).toBe('single');
    ...
    expect(window.localStorage.getItem('read-buddy:page-mode')).toBe('double');
    await waitFor(() =>
      expect(engine.applyPresentation).toHaveBeenLastCalledWith(
