...
const databases: ReadBuddyDatabase[] = [];
...
const makeStore = (
  ...
  const db = new ReadBuddyDatabase(`chat-tab-test-${Math.random().toString(36).slice(2)}`);
  databases.push(db);
...
describe('ChatTab', () => {
  ...
  it('renders the collapsed tool-trace accordion and citation cards on a reply', async () => {
    ...
    window.addEventListener('read-buddy:agent-locate-test', locateSpy);
    ...
    window.removeEventListener('read-buddy:agent-locate-test', locateSpy);
  });
  ...
  it('focuses the chat input with the caret at the end on the selection "ask" event', async () => {
    ...
    window.dispatchEvent(new Event('read-buddy:focus-chat-input'));
