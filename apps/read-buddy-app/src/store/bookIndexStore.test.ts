...
const makeHarness = (options: { stream?: StreamTextFn; db?: ReadBuddyDatabase; spineAsync?: boolean; unstructured?: boolean } = {}): Harness => {
  const db = options.db ?? new ReadBuddyDatabase(`index-store-test-${Math.random().toString(36).slice(2)}`);
