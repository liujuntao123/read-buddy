...
const makeStore = (
  ...
  const db = new ReadBuddyDatabase(`chat-store-test-${Math.random().toString(36).slice(2)}`);
  databases.push(db);
...
function makeFailingStore() {
  const db = new ReadBuddyDatabase(`chat-store-fail-${Math.random().toString(36).slice(2)}`);
  databases.push(db);
