...
const STORAGE_KEY = 'read-buddy:reader-settings';
...
const clampLayout = (l: Partial<ReaderLayoutSettings>): ReaderLayoutSettings => {
  ...
  if (l.pageMode === 'single' || l.pageMode === 'double') {
    ...
    try {
      const legacy = window.localStorage.getItem('read-buddy:page-mode');
...
export const useReaderSettingsStore = create<ReaderSettingsState>((set, get) => ({
  ...
  setPageMode: (pageMode) => {
    ...
    writeStoredSettings({ typography: get().typography, layout });
    try {
      window.localStorage.setItem('read-buddy:page-mode', pageMode);
    } catch {
      /* ignore */
    }
  },
  ...
  reset: () => {
    ...
    try {
      window.localStorage.setItem('read-buddy:page-mode', DEFAULT_LAYOUT_SETTINGS.pageMode);
    } catch {
      /* ignore */
    }
  },
