...
class View {
    ...
    expand() {
        // read-buddy: a view whose iframe has been removed (a page turn, a
...
export class Paginator extends HTMLElement {
    static observedAttributes = [
        ...
        // read-buddy: continuous scrolled reading (several chapters stacked in
    ...
    #touchScrolled
    #lastVisibleRange
    // read-buddy: continuous scrolled mode state. `#entries` are the live
    ...
    constructor() {
        ...
        :host([flow="scrolled"]) #container {
            ...
            grid-row: 1 / -1;
            overflow: auto;
            /* read-buddy: 单页（scrolled）模式隐藏滚动条。容器仍是原生可滚动
               区域（滚轮 / 触控板 / 翻页键行为不变），只是不再画那条系统滚动条；
               这一块在 shadow DOM 里，globals.css 的全局滚动条样式够不到它。 */
            scrollbar-width: none;
        }
        /* read-buddy: WebKit / Chromium（Tauri = WebView2）侧的隐藏。 */
        ...
        this.#container.addEventListener('scroll', () => {
            ...
            // read-buddy: keep the continuous flow fed while the reader scrolls.
    ...
    attributeChangedCallback(name, _, value) {
        switch (name) {
            case 'flow':
                ...
                break
            case 'continuous':
                // read-buddy: entering/leaving the continuous flow rebuilds (or
    ...
    // ---------------------------------------------------------------------
    // read-buddy: continuous scrolled reading
    //
    ...
    render() {
        ...
        // read-buddy: every live chapter shares one relayout in continuous mode
    ...
    async #display(promise) {
        ...
        if (src) {
            // read-buddy: the replaced chapter's document is going away; say so,
    ...
    async #goTo({ index, anchor, select}) {
        // read-buddy: in the continuous flow a chapter is reached by scrolling
    ...
    #scrollPrev(distance) {
        ...
        // read-buddy: continuous flow — a page turn is a scroll, never a jump to
    ...
    getContents() {
        // read-buddy: every live chapter is a content, so overlays, CFI lookups
    ...
    setStyles(styles) {
        this.#styles = styles
        // read-buddy: in continuous mode the stylesheet has to reach every live
