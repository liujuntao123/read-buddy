...
describe('highlightSnippet', () => {
  it('wraps the quoted snippet in breathing marks and injects styles once', () => {
    ...
    expect(document.querySelectorAll('style#readest-agent-highlight-style').length).toBe(1);
    expect(document.getElementById('readest-agent-highlight-style')?.textContent).toContain(
      'readest-agent-breathe',
    );
