...
2. **The originating spec is inconsistent too.** The design doc declared the field as `sectionIndex` (both early design docs are now deleted — `docs/read-buddy 桌面端应用设计文档.md` and `docs/technical-solution-reading-agent-architecture.md`; see git history), and `readerStore` already renamed its own `nodeIndex` → `spineIndex` for the same reason (ADR 0010 ¶6). The persisted rows are the one place the rename was never carried out.
...
4. **No migration precedent existed.** `ReadBuddyDatabase` had five `.version()` calls and no `.upgrade()` body anywhere; v5 chose to drop and rebuild rather than migrate. These are the first data-preserving upgrades, so the pattern they set matters.
