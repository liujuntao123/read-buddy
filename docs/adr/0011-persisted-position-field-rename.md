...
2. **The originating spec is inconsistent too.** The design doc declared the field as `sectionIndex` (both early design docs are now deleted — `docs/readest-plus 桌面端应用设计文档.md` and `docs/technical-solution-reading-agent-architecture.md`; see git history), and `readerStore` already renamed its own `nodeIndex` → `spineIndex` for the same reason (ADR 0010 ¶6). The persisted rows are the one place the rename was never carried out.
